...
export default function createPayload(submission: t.TSubmission) {
  ...
  // Normalize conversation object for Solid storage compatibility
  // Solid storage may return messages as objects instead of IDs, and null values instead of undefined
  // Wrap in try-catch to ensure we can always fall back if normalization fails
  let normalizedConversation: Record<string, unknown>;
  try {
    normalizedConversation = { ...conversation };
    
    // Convert messages array from objects to IDs if needed
    ...
    // If normalization fails, just use the original conversation and let the fallback handle it
    normalizedConversation = conversation as Record<string, unknown>;
  }
  
  // Convert ALL null values to undefined for optional fields that expect specific types
  // This is necessary because Zod schemas use .optional() which means undefined, not null
  ...
  // safeParse never throws, it always returns { success: true, data } or { success: false, error }
  const parseResult = s.tConvoUpdateSchema.safeParse(normalizedConversation);
  
  if (!parseResult.success) {
    // Fallback: try to extract conversationId directly if parse fails
    // For new conversations, conversationId can be null or 'new', which is valid
  ...
  if (s.isAssistantsEndpoint(endpoint)) {
    ...
    // Extract important fields from the conversation object and endpointOption
    // This is especially important for Solid storage where the conversation might have the model
    // Check endpointOption first, then conversation, as endpointOption takes precedence
  ...
  // Extract model from conversation if not in endpointOption (for Solid storage compatibility)
